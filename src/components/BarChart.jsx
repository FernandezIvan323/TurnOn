export default function BarChart({ data, valueKey = "value", labelKey = "label", maxBars = 20, height = 180 }) {
  if (!data || data.length === 0) {
    return <div className="text-sm text-ink-400 dark:text-ink-500 text-center py-6">Sin datos en el período.</div>;
  }
          ...
          <div key={i} className="flex items-center gap-3 text-xs">
            ...
            <div className="flex-1 bg-paper-200 dark:bg-ink-800 rounded-lg overflow-hidden h-5 relative">
              <div
                ...
                style={{ width: `${pct}%` }}
              />
              <span className="absolute right-2 top-0 leading-5 text-ink-600 dark:text-ink-300 font-medium">
