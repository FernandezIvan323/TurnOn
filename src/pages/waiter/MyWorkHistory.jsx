...
function DriverView() {
  ...
  const loadDays = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get("/reports/my-deliveries");
      setDays(data || []);
    } catch (e) {
      setError(e.response?.data?.error || e.message);
    } finally {
  ...
  const openDay = async (date) => {
    ...
    setDetailLoading(true);
    try {
      const { data } = await api.get(`/reports/my-deliveries/${date}`);
      setDetail(data);
    } catch (e) {
      setError(e.response?.data?.error || e.message);
    } finally {
  ...
  const closeDay = () => { setSelectedDate(null); setDetail(null); };
  const isToday = selectedDate === todayLocalISO();
      ...
      <div>
        ...
        {detail && !detailLoading && (
          <>
            <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              <div className="card p-3">
                <div className="text-[11px] text-ink-500">Entregas</div>
                <div className="text-xl font-bold text-ink-900 dark:text-white">{detail.summary.orders_count}</div>
              </div>
              <div className="card p-3">
                <div className="text-[11px] text-ink-500">Cobrado</div>
              ...
              <div className="card border-l-4 border-l-emerald-500 p-3">
                <div className="text-[11px] text-emerald-700 dark:text-emerald-300">Efectivo a rendir</div>
                <div className="text-xl font-bold text-emerald-700 dark:text-emerald-300">{money(detail.summary.cash_sales)}</div>
              </div>
              <div className="card p-3">
                <div className="text-[11px] text-ink-500">Transferencia</div>
                <div className="text-xl font-bold text-ink-900 dark:text-white">{money(detail.summary.transfer_sales)}</div>
              </div>
            </div>
            {detail.orders.length === 0 ? (
              <div className="card p-8 text-center text-sm text-ink-500">Sin entregas este día.</div>
            ) : (
              <div className="space-y-3">
                {detail.orders.map((o) => {
                    ...
                    <div key={o.id} className="overflow-hidden rounded-xl border border-paper-300 dark:border-obsidian-700">
                      <div className="flex flex-wrap items-center justify-between gap-2 bg-paper-100 px-3 py-2 text-xs font-medium text-ink-800 dark:bg-obsidian-800 dark:text-white">
                        ...
                        <span className={`badge text-[10px] ${
                          ...
                          {isDebt ? "Deuda" : isCash ? "Cobrado" : "Transfer."}
                        </span>
                      </div>
                      <div className="space-y-1.5 px-3 py-2">
                        <div className="flex items-center gap-2 text-sm font-semibold text-ink-800 dark:text-white">
                          <Truck size={14} className="shrink-0 text-wine-600" />
                          {o.customer_name || "Cliente"}
                        </div>
                        {(o.customer_address || o.customer_neighborhood) && (
                          <div className="text-xs text-ink-600 dark:text-obsidian-300">
  ...
  return (
    <div>
      <Header
        title="Mi historial"
        subtitle="Entregas por día — efectivo a rendir"
      />
      {error && (
        <div className="mb-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:border-rose-800 dark:bg-rose-900/30 dark:text-rose-300">
          {error}
      ...
      {loading ? (
        ...
        <div className="card border-dashed p-10 text-center">
          <Truck size={32} className="mx-auto mb-2 text-ink-400" />
          <div className="font-semibold text-ink-800 dark:text-white">Aún no entregaste pedidos</div>
          <p className="mt-1 text-sm text-ink-500">Cuando completes entregas, aparecerán acá por día.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {days.map((d) => {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold capitalize text-ink-900 dark:text-white">{formatDayLabel(d.date)}</span>
                    {today && (
                      <span className="rounded-md bg-blue-50 px-1.5 py-0.5 text-[10px] font-bold uppercase text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">Hoy</span>
                    )}
                  </div>
                  <div className="mt-0.5 text-xs text-ink-500 dark:text-obsidian-400">
                    {d.orders_count} entrega{d.orders_count === 1 ? "" : "s"}
                    {" · "}Cobrado {money(d.total_sales)}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[11px] text-emerald-700 dark:text-emerald-300">Efectivo</div>
                  <div className="font-bold tabular-nums text-emerald-700 dark:text-emerald-300">{money(d.cash_sales)}</div>
                  <ChevronRight size={16} className="ml-auto mt-0.5 text-ink-400" />
                </div>
              </button>
            );
