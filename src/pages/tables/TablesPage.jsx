...
return (
    <DetailModal
      ...
      badge="Mesa"
      type="table"
      amount={order ? money(total) : undefined}
      status={order ? statusBanner(order) : undefined}
      onClose={onClose}
    >
      <div className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-ink-600 dark:text-obsidian-300">
        <span>
          Mesero:{" "}
          <b className="text-ink-900 dark:text-white">{table.assigned_user_name || "Sin asignar"}</b>
        </span>
        {order && (
          <span>
      ...
      {order && order.status === "ready_to_pay" && (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 dark:border-amber-800 dark:bg-amber-900/20">
      ...
      {!order ? (
        ...
        >
          ...
          {!isAdmin && (
            <div className="bg-paper-200/40 p-4 dark:bg-obsidian-950 sm:p-5">
...
<div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
      ...
      {order && (
        <div className="mt-4 flex flex-wrap items-center justify-end gap-2 border-t border-paper-300 pt-3 dark:border-obsidian-800">
...
{loading && tables.length === 0 ? (
      ...
      ) : isAdmin && viewMode === "byWaiter" && waiterGroups ? (
