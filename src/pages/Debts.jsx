...
export default function Debts() {
  ...
  const payDebt = async (id) => {
    setPaying(id);
    try {
      await api.post(`/orders/${id}/pay-debt`, { payment_method: payMethod });
      await load();
    } catch (e) {
  ...
  return (
    <div>
      ...
      {tab === "pending" && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 dark:border-rose-800 dark:bg-rose-900/20">
          <div className="flex items-center gap-2 text-rose-800 dark:text-rose-200">
            <AlertTriangle size={20} />
            <span className="font-semibold">
              {filtered.length} deuda{filtered.length === 1 ? "" : "s"} pendiente
              {filtered.length === 1 ? "" : "s"}
            </span>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <label className="flex items-center gap-2 text-xs text-rose-800 dark:text-rose-200">
              Cobrar con
              <select
      ...
      {tab === "settled" && (
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-200">
          <History size={16} />
          Historial de deudas cobradas (desde esta versión del sistema). Total listado:{" "}
          <b>{money(totalDebt)}</b>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-ink-500 dark:text-obsidian-400">
          ...
          {tab === "pending" ? "No hay deudas pendientes." : "Aún no hay deudas cobradas registradas."}
        </div>
      ) : (
            ...
            <div key={o.id} className="card flex items-center justify-between gap-4 p-4">
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  <span className="badge bg-slate-100 text-[10px] text-slate-700 dark:bg-obsidian-800 dark:text-obsidian-200">
                    #{o.id}
                  </span>
                    ...
                    <span
                      className={`badge text-[10px] ${
                        daysSince(o.closed_at) >= 7
                          ...
                          : "bg-slate-100 text-slate-600 dark:bg-obsidian-800 dark:text-obsidian-300"
                      }`}
                    >
              ...
              <div className="text-right">
                <div className="text-lg font-bold tabular-nums text-ink-800 dark:text-obsidian-50">
                  {money(o.total)}
                </div>
                {tab === "pending" && (
                  <button
